...
One approach to the authoring task is to compose rhythmic sequences as types of musical compositions. The text-based music notation software Lilypond \cite{nienhuys2003lilypond} could be used for this purpose. Output of midi events from composition defined in Liypond could then up uploaded directly onto the Arduino micro-controller board using the Arduino midi library \cite{best2014arduinoMidi}. As noted in \cite{lee2010buzzwear} that temporal pattern is the most easily distinguishable parameter for wrist tactile patterns is may make sense to involve musical approaches, which also deal with temporal patterning. This integration of well-established musical composition techniques with haptic patterning seems especially promising. 
